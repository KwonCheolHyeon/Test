...
namespace gui
{
	...
	void Texture::Update()
	{
		md::graphics::Texture* targetTex 
			= dynamic_cast<md::graphics::Texture*>(GetTarget());
		...
		std::string key =  WStringToString(targetTex->GetKey());
		std::string path = WStringToString(targetTex->GetPath());
		...
		int width = static_cast<int>(targetTex->GetWidth());
		int height = static_cast<int>(targetTex->GetHeight());
