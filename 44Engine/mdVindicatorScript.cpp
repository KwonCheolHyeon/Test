...
namespace md
{
	VindicatorScript::VindicatorScript()
		: Script()
		, mState(eVindicatorState::Idle)
		, mTr(nullptr)
		, mRb(nullptr)
		, mTarget(nullptr)
		, mMoveSpeed(3.0f)
	...
	void VindicatorScript::Initalize()
	{
		MeshData* meshData = MeshData::LoadFromFbx(L"fbx\\Vindicator.fbx");
		if (meshData)
		{
		...
		//
		//BaseRenderer* vidicatorBaseRenderer = GetOwner()->GetComponent<BaseRenderer>();
		//material = Resources::Find<Material>(L"DefferdMaterial");
		//vidicatorBaseRenderer->SetMaterial(material,0);
		//material = Resources::Find<Material>(L"OutLineMaterial");
		//vidicatorBaseRenderer->SetMaterial(material, 1);
	...
	void VindicatorScript::Update()
	{
		...
		switch (mState)
		{
		case eVindicatorState::Idle:
			Idle();
			break;

		case eVindicatorState::Walk:
			Walk();
			break;

		case eVindicatorState::Stun:
			Stun();
			break;

		case eVindicatorState::NoveltySleep:
			NoveltySleep();
			break;

		case eVindicatorState::BasicAttack:
			BasicAttack();
			break;

		/*case eVindicatorState::GetHitFromTheFront:
			GetHitFromTheFront();
			break;*/
	...
	void VindicatorScript::Render()
	{
		
	}
	void VindicatorScript::OnCollisionEnter(Collider3D* collider)
	{
		if (collider->GetOwner()->GetName() == L"Player")
		{
			...
			mdOnCollision = true;
			//mState = eVindicatorState::GetHitFromTheFront;
		}
	}
	void VindicatorScript::OnCollisionStay(Collider3D* collider)
	{
	}
	void VindicatorScript::OnCollisionExit(Collider3D* collider)
	{
		if (collider->GetOwner()->GetName() == L"Player")
		{
			mdOnCollision = false;
	...
	void VindicatorScript::Start()
	{
	}
	void VindicatorScript::Action()
	{
	}
	void VindicatorScript::End()
	{
	}
	void VindicatorScript::Idle()
	{
		if (!mTarget)
		{
		...
		if (distance <= 10.f)
		{
			mState = eVindicatorState::Walk;
		}
	}
	void VindicatorScript::Walk()
	{
		if (!mTarget)
		{
			mTarget = SceneManager::FindPlayerObject();
	...
	void VindicatorScript::Stun()
	{
	}

	void VindicatorScript::NoveltySleep()
	{
	}

	void VindicatorScript::BasicAttack()
	{
		if (!mdOnCollision)
			mState = eVindicatorState::Walk;
	}
	void VindicatorScript::GetHitFromTheFront()
	{

	}

	void VindicatorScript::RotateTowardsTarget(math::Vector3 _pos, math::Vector3 _targetPos)
	{
