...
namespace md::graphics
{
	...
	void Texture::Clears()
	{
		...
		{
			UINT castedSlotNum = static_cast<UINT>(i);

			ID3D11ShaderResourceView* srv = nullptr;
			GetDevice()->BindShaderResource(eShaderStage::VS, castedSlotNum, &srv);
			GetDevice()->BindShaderResource(eShaderStage::DS, castedSlotNum, &srv);
			GetDevice()->BindShaderResource(eShaderStage::GS, castedSlotNum, &srv);
			GetDevice()->BindShaderResource(eShaderStage::HS, castedSlotNum, &srv);
			GetDevice()->BindShaderResource(eShaderStage::CS, castedSlotNum, &srv);
			GetDevice()->BindShaderResource(eShaderStage::PS, castedSlotNum, &srv);
		}
	}

	bool Texture::Create(UINT width, UINT height, DXGI_FORMAT format, UINT bindFlag, bool srv)
	{
		if (mTexture == nullptr)
		{
		...
		if (bindFlag & D3D11_BIND_FLAG::D3D11_BIND_SHADER_RESOURCE || srv == true)
		{
